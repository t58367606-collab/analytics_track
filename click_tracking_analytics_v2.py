...
async def unified_report():
    ...
    try:
        ...
        # ── Summary by concept ────────────────────────────────────────────────
        # combined_score = engagement_score + link_clicks × 5
        ...
        for r in report:
            ...
            concept_summary[ck]['total_link_clicks']      += r['link_clicks']
            concept_summary[ck]['total_engagement_score'] += eng
            concept_summary[ck]['combined_score']         += eng + r['link_clicks'] * 5
            concept_summary[ck]['platforms'].add(r['platform'])
            if ct == 'image':
            ...
            if ct in type_summary:
                ...
                type_summary[ct]['total_clicks']     += r['link_clicks']
                type_summary[ct]['total_engagement'] += eng

        concept_summary_list = sorted([
            {
                'concept_key':            ck,
